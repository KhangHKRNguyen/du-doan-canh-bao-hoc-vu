...
if st.button("Chạy dự đoán"):
    ...
    # 5. HIỂN THỊ KẾT QUẢ
    st.divider()
    st.markdown(f"### Kết quả dự đoán: :{colors[pred]}[{labels[pred]}]")
    st.write(f"**Độ tin cậy:** {np.max(prob)*100:.2f}%")
    
    # Vẽ biểu đồ cột xác suất cho chuyên nghiệp
    chart_data = pd.DataFrame(prob, index=["Bình thường", "Cảnh báo", "Thôi học"], columns=["Xác suất"])
